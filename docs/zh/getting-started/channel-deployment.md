...
2. 设置 **Messaging endpoint** 为你的 AI 员工的 MS Teams webhook URL：
   ...
   ```
   你可以在 OpenMax Dashboard 员工详情页的渠道列表中，**Microsoft Teams** 卡片上找到该 URL。复制并粘贴到此处。
...
4. 输入描述（如 `OpenMax Bot Secret`），选择过期时间
5. 点击 **Add**
...
3. 输入应用名称（如 `OpenMax AI Employee`）
4. 在 **Basic information** 中，记下顶部显示的 **App ID** — 这就是你的 **App Catalog ID**（连接 OpenMax Dashboard 时需要）
5. 填写必填字段：
   - **Short description**：如 `AI 智能数字员工`
   - **Long description**：如 `OpenMax AI 员工，帮助团队处理写作、研究、翻译、数据分析和日常任务——直接在 Microsoft Teams 中使用。`
   - **Developer name**：如 `OpenMax`
...
1. 在 Teams 中搜索你的应用名称（如 `OpenMax AI Employee`）
2. 点击即可开始私聊对话
3. 发送任何消息 — AI 员工即时响应
4. 部署完成！
...
| 客户端密钥过期 | Azure 客户端密钥会按你设置的时间过期。创建新密钥并在 OpenMax Dashboard 中更新 App Password |
